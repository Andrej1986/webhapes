...
<div class="site-cart">
        ...
        <div class="cart-item cart-item-<?= $product['id'] ?>">
            ...
            <div class="item-second-row">
                <h4 class="col-sm-6 total item-<?= $product['id'] ?>-total">
                    ...
                    <span><?= (($product['category'] !== 'na mieru' ? Yii::$app->session->get('item_' . $product['id']) : 1) * $product['price']) ?></span>
                    €<?= $product['category'] === 'na mieru' ? '/hod' : '' ?>
                </h4>
    ...
    <a href="<?= Url::to(['//site/shop'])?>" class="btn btn-confirm">Prejsť k objednávke</a>

    <h3>Celkom na zaplatenie</h3>
    <h4>Platba vopred: <strong> <?= $sum ?></strong> €</h4>
	<?php if (Yii::$app->session->get('na_mieru')): ?>
        <h4>Pri hodinovej sadzbe sa konečná cena stanoví až na základe reálne odpracovaných hodín. </h4>
	<?php endif; ?>


	<?= Html::a('Pokračovať v nákupe', ['//site/shop'], ['class' => 'btn']) ?>
